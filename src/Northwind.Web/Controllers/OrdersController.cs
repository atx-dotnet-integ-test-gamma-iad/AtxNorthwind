...
public class OrdersController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAllOrders()
    {
        var orders = await _context.Database
            .SqlQueryRaw<Order>("SELECT * FROM Orders WHERE OrderDate >= CURRENT_TIMESTAMP - INTERVAL '30 days'")
            .ToListAsync();
        return Ok(orders);
    }
    ...
    public async Task<IActionResult> GetOrdersByCustomer(string customerId)
    {
        ...
        var orders = await _context.Database
            .SqlQueryRaw<Order>(
                "SELECT *, CAST(EXTRACT(epoch FROM CAST(CURRENT_TIMESTAMP AS TIMESTAMP) - CAST(OrderDate AS TIMESTAMP)) / 86400 AS BIGINT) AS DaysAgo FROM Orders WHERE CustomerID = @CustomerID",
                param)
            .ToListAsync();
        return Ok(orders);
    }
