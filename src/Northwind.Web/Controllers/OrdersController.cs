...
public class OrdersController : ControllerBase
{
    ...
    {
        var param = new NpgsqlParameter("$1", customerId);
        var orders = await _context.Database
            .SqlQueryRaw<Order>(
                "SELECT *, EXTRACT(DAY FROM (NOW() - OrderDate))::int AS DaysAgo FROM northwind_dbo.orders WHERE CustomerID = $1",
                param)
            .ToListAsync();
        return Ok(orders);
    }
    ...
    public async Task<IActionResult> GetCustomerOrderHistory(string customerId)
    {
        ...
        var result = await _context.Database
            .SqlQueryRaw<OrderHistoryResult>(
                "SELECT * FROM northwind_dbo.custorderhist($1)",
                param)
            .ToListAsync();
        return Ok(result);
    }
