...
public class OrdersController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAllOrders()
    {
        var orders = await _context.Database
            .SqlQueryRaw<Order>("SELECT * FROM public.\"Orders\" WHERE \"OrderDate\" >= CURRENT_TIMESTAMP - INTERVAL '30 days'")
            .ToListAsync();
        return Ok(orders);
    }
    ...
    public async Task<IActionResult> GetOrdersByCustomer(string customerId)
    {
        ...
        var orders = await _context.Database
            .SqlQueryRaw<Order>(
                "SELECT *, CAST(EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - \"OrderDate\"::TIMESTAMP)) / 86400 AS BIGINT) AS DaysAgo FROM public.\"Orders\" WHERE \"CustomerID\" = @CustomerID",
                param)
            .ToListAsync();
        return Ok(orders);
    }
    ...
    public async Task<IActionResult> GetRecentOrders()
    {
        var orders = await _context.Database
            .SqlQueryRaw<Order>(
                ...
                "FROM public.\"Orders\" WHERE \"ShippedDate\" IS NOT NULL")
            .ToListAsync();
        return Ok(orders);
    }
}
