...
public class OrdersController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAllOrders()
    {
        var orders = await _context.Database
            .SqlQueryRaw<Order>("SELECT * FROM Orders WHERE OrderDate >= NOW() - INTERVAL '30 days'")
            .ToListAsync();
        return Ok(orders);
    }
    ...
    public async Task<IActionResult> GetOrdersByCustomer(string customerId)
    {
        ...
        var orders = await _context.Database
            .SqlQueryRaw<Order>(
                "SELECT *, DATE_PART('day', NOW() - OrderDate)::INTEGER AS DaysAgo FROM Orders WHERE CustomerID = @CustomerID",
                param)
            .ToListAsync();
        return Ok(orders);
    }
