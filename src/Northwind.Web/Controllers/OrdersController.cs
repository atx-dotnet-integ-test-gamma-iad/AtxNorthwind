...
public class OrdersController : ControllerBase
{
    ...
    public async Task<IActionResult> GetOrdersByCustomer(string customerId)
    {
        ...
        var orders = await _context.Database
            .SqlQueryRaw<Order>(
                "SELECT *, CAST(EXTRACT(epoch FROM CURRENT_TIMESTAMP - OrderDate::TIMESTAMP) / 86400 AS INTEGER) AS DaysAgo FROM northwind_dbo.orders WHERE CustomerID = @CustomerID",
                param)
            .ToListAsync();
        return Ok(orders);
    }
