...
public class OrdersController : ControllerBase
{
    ...
    public async Task<IActionResult> GetOrdersByCustomer(string customerId)
    {
        ...
        var orders = await _context.Database
            .SqlQueryRaw<Order>(
                "SELECT *, CAST(EXTRACT(epoch FROM CAST(CAST(CURRENT_TIMESTAMP AS TIMESTAMP) AS TIMESTAMP) - CAST(CAST(OrderDate AS TIMESTAMP) AS TIMESTAMP)) / 86400 AS BIGINT) AS DaysAgo FROM northwind_dbo.orders WHERE CustomerID = @CustomerID",
                param)
            .ToListAsync();
        return Ok(orders);
    }
