...
public class OrdersController : ControllerBase
{
    ...
    public async Task<IActionResult> GetOrdersByCustomer(string customerId)
    {
        ...
        var orders = await _context.Database
            .SqlQueryRaw<Order>(
                "SELECT *, DATE_PART('day', CURRENT_TIMESTAMP - OrderDate)::BIGINT AS DaysAgo FROM Orders WHERE CustomerID = @CustomerID",
                param)
            .ToListAsync();
        return Ok(orders);
    }
    ...
    public async Task<IActionResult> GetRecentOrders()
    {
        var orders = await _context.Database
            .SqlQueryRaw<Order>(
                ...
                "EXTRACT(YEAR FROM OrderDate)::TEXT AS OrderYear " +
