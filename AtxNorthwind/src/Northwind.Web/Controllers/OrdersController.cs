...
public class OrdersController : ControllerBase
{
    ...
    {
        var param = new NpgsqlParameter("@p1", customerId);
        var orders = await _context.Database
            .SqlQueryRaw<Order>(
                "SELECT *, CAST(EXTRACT(epoch FROM CURRENT_TIMESTAMP - OrderDate::TIMESTAMP) / 86400 AS BIGINT) AS DaysAgo FROM northwind_dbo.orders WHERE CustomerID = @p1",
                param)
            .ToListAsync();
        return Ok(orders);
    }
